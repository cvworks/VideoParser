...
public:
	...
	T value(float,float) const;		//bilinearly interpolated value anywhere inside field 
	...
	void		minmaxTREESKEL(T&,T&,T&) const;		//min, max, avg for field
	...
	void		writecompactfieldtofile(char*) const;
	void		writefieldtofile(char*) const;		//write field to a text file containing all the values in a grid as is
	void		writecnttofile(char*) const;
...
private:
	...
	static FIELD* readPPM(char*);				//read field from PGM binary file
...
public:
	...
	void		write(const char*,FIELD<T>&) const;
...
template <class T> inline T FIELD<T>::value(float i,float j) const
{
...
cout << "v[0] " << v[0] << endl;
M = -T(INFINITY);
cout << "-T(INFINITY) " << -T(INFINITY) << endl;
a = 0;
...
template <class T> void FIELD<T>::minmaxTREESKEL(T& m,T& M,T& a) const
{
	const float INFINITY_2 = INFINITY/2;
	...
	M = -T(INFINITY);
	std::cout << "-T(INFINITY) " << -T(INFINITY) << std::endl;
	a = 0;
...
template <class T> unsigned long FIELD<T>::getLong(std::fstream& inf)
{
...
template <class T> unsigned short FIELD<T>::getShort(std::fstream& inf)
{
	...
	for(unsigned int row=0;row<numRows;row++)               //for every row     
	{
		for(unsigned int col=0;col<numCols;col++)
		{
			...
			{  
				...
				//--------------------------------ADDED---------------------------------------------------
				unsigned short b = getShort(inf);
				unsigned short g = getShort(inf);
				unsigned short r = getShort(inf);

				*data++ = (int(r) + int(g) + int(b)) / 3.0f;
				//-------------------------------~ADDED----------------------------------------------------	    
			}
			...
			{  
				//inf.get(ch); *data++ = ch; 
				//--------------------------------ADDED---------------------------------------------------
				unsigned short ch1 = getShort(inf);
				*data++ = ch1;
				//-------------------------------~ADDED----------------------------------------------------		
			}
		}
...
template <class T> FIELD<T>* FIELD<T>::readPPM(char* fname)	//read VTK scalar file into this
{
	FILE* fp = fopen(fname,"r"); if (!fp) return 0;

	const int SIZE = 1024;
	char buf[SIZE]; int dimX,dimY,range;
	fscanf(fp,"%*s");				//skip "P6" header
...
template <class T> void FIELD<T>::writecnttofile(char* fname) const
//write the raw values of the field into a text file
{
	FILE* fp = fopen(fname, "w");
	if(!fp) return;
...
template <class T> void FIELD<T>::writecompactfieldtofile(char* fname) const
{
	int x, y, xmin = 100000, xmax = 0, ymin = 100000, ymax = 0;
...
template <class T> void FIELD<T>::writefieldtofile(char* fname) const
//write the raw values of the field into a text file
{
	FILE* fp = fopen(fname, "w");
	if(!fp) return;
...
template <class T> void VFIELD<T>::write(const char* fname,FIELD<T>& f) const
{
	FILE* fp = fopen(fname,"w");
	if (!fp) return;
