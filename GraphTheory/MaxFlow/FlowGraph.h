...
/*
	...
	trees from one maxflow computation to the next, as described in
		...
		"Efficiently Solving Dynamic Markov Random Fields Using FlowGraph Cuts."
...
public:
	...
	FlowGraph(int node_num_max, int edge_num_max, void (*err_function)(char *) = NULL);
	...
	// segment the node 'i' belongs (FlowGraph<captype,tcaptype,flowtype>::SOURCE or FlowGraph<captype,tcaptype,flowtype>::SINK).
	//
...
public:
	...
	//
	// Advantage compared to deleting FlowGraph and allocating it again:
	...
	// Example usage:
	//
	//		typedef FlowGraph<int,int,int> G;
	//		G* g = new FlowGraph(nodeNum, edgeNum);
