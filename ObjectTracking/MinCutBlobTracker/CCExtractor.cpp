...
void CCExtractor::process(const Mat &binaryImage)
{
	...
	//DBG_PRINT2(components.size(), connectedComponents.size())
...
void CCExtractor::extractConnectedComponents()
{
    ...
    for (EquivalenceClassList::iterator ecList = equivalenceClassList.begin(), ecListEnd = equivalenceClassList.end(); ecList != ecListEnd; ecList++)
    {
        ...
        comp.reset(new ConnectedComponent(id, *ecList));
...
{
    ComponentPtr newComponent(new Component(componentCount, p));
    EquivalenceClassPtr NewEquivalenceClass(new EquivalenceClass());
...
void CCExtractor::relabelImage()
{
    ...
    auto rcc = rejectedConnectedComponents.begin(), rccEnd = rejectedConnectedComponents.end();
    ...
    auto cc = connectedComponents.begin(), ccEnd = connectedComponents.end();
...
void CCExtractor::setEquivalence(ComponentPtr comp1, ComponentPtr comp2)
{
    EquivalenceClassPtr eq1 = comp1->eqClass;
    EquivalenceClassPtr eq2 = comp2->eqClass;

    if (eq1 == eq2) return;   // Equivalence has already been established

    EquivalenceClassPtr l;
    EquivalenceClassPtr s;
...
void CCExtractor::outputComponents()
{
    ...
    auto it = components.begin(), itEnd = components.end();
...
void CCExtractor::outputEquivalenceClass(EquivalenceClassPtr eqClass)
{
