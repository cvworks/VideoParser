...
*/
#define TypeName(T) std::string(typeid(T).raw_name())
...
#define DECLARE_BASIC_SERIALIZATION(T) \
	...
	inline void Serialize(cv::FileStorage& os, const T& x) \
	{ \
		os.write((char*) &x, sizeof(x)); \
	} \
	inline void Deserialize(cv::FileStorage& is, T& x) \
	{ \
		is.read((char*) &x, sizeof(x)); \
	} \
...
template<typename OS, typename T, typename U>
inline void Serialize(OS& os, const std::pair<T, U>& P) 
{
	Serialize(os, P.first);
	Serialize(os, P.second);
}

template<typename IS, typename T, typename U>
inline void Deserialize(IS& is, std::pair<T, U>& P) 
{
...
inline void Serialize(OS& os, const std::vector<T>& c)
{
	Serialize(os, c.size());
...
template<typename OS, typename T> 
inline void Serialize(OS& os, const std::list<T>& c)
{
	Serialize(os, c.size());
...
template<typename OS, typename T, typename U> 
inline void Serialize(OS& os, const std::map<T,U>& c)
{
	Serialize(os, c.size());
...
inline void Deserialize(IS& is, std::vector<T>& C) 
{
	unsigned sz;
...
template<typename IS, typename T>
inline void Deserialize(IS& is, std::list<T>& C) 
{
	unsigned sz;
...
template<typename IS, typename T, typename U>
inline void Deserialize(IS& is, std::map<T, U>& C) 
{
	unsigned sz;
...
template <typename OS, typename T>
inline void Serialize(OS& os, const T& x)
{
	x.Serialize(os);
}

template <typename IS, typename T>
inline void Deserialize(IS& is, T& x)
{
	x.Deserialize(is);
}
